...
generate_ssh_key() {
    ...
    if [ -f "$rsa_key" ]; then
        ...
        chown -R $local_username:$local_username "$ssh_dir"
    fi
}
...
copy_ssh_key() {
    ...
    if command -v ssh-copy-id &> /dev/null; then
        ...
        ssh "$target_username@$ip_address" "mkdir -p ~/.ssh && chmod 700 ~/.ssh && echo \"$public_key\" >> ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys"
    fi
...
# Prompt for the local and target server's username and IP address
...
copy_ssh_key "$local_username" "$target_username" "$target_ip_address"
